...
os.environ["OPENAI_API_KEY"] = (
    "sk-PkpQcEsH60fcJae58UJIgSOl_gRtopiwXgkn1V9SrMT3BlbkFJt89WWtVaxiwb25Bsy_rHjUpEn0ttr2Y9rhoBmjyl8A"
)
...
with st.sidebar:
    ...
    with st.form(key="research_form"):
        topic = st.text_input("Enter the ticket stock")
        submit_button = st.form_submit_button(label="Run Research")

    if submit_button:
        if not topic:
            st.warning("Please enter a stock ticket")
        else:
            results = crew.kickoff(inputs={"ticket": topic})

            st.subheader("Results of the Research")
